...
const FormContainer = styled.form<FormContainerProps>`
	...
	opacity: ${(props) => {
		if ((props.screenWidth <= 500 && props.pageYOffset >= 3100) || (props.screenWidth <= 860 && props.pageYOffset >= 3400) || (props.screenWidth > 860 && props.pageYOffset >= 4000)) {
			return 1;
		}
		return 0;
	}};
			...
			{isThanksRendered ? (
				...
				<FormContainer textAreaHeight={textAreaRef?.current?.offsetHeight} ref={form} onSubmit={sendEmail} pageYOffset={pageYOffset} screenWidth={screenWidth} data-testid="form-submit">
					...
					<Label htmlFor="name">
						...
						<Input type="text" placeholder="Full Name" name="user_name" autoComplete="off" required data-testid="form-name" />
					</Label>
					...
					<Label htmlFor="email">
						...
						<Input type="email" placeholder="example@email.com" name="user_email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" autoComplete="off" required data-testid="form-email" />
					</Label>
					...
					<Label htmlFor="message">
						...
						<TextArea ref={textAreaRef} placeholder="Write your message here" onChange={(event) => setMessageInput(event?.target.value)} rows={1} name="message" value={messageInput} minLength={30} required data-testid="form-message" />
					</Label>
